...
{
	public sealed class EmbeddedTemplateExportItem
	{
	...
	public sealed class EmbeddedTemplateExportResult
	{
	...
	/// <summary>
	/// Copies embedded Razor templates under <c>Scaffolding/Templates</c>
	/// into <c>.forge/Templates</c>.
	/// </summary>
	public sealed class EmbeddedTemplateExporter
	{
		private const string ForgeFolder = ".forge";
		private const string TemplatesFolder = "Templates";
		private const string ResourceSegment = "Scaffolding.Templates.";

		public IReadOnlyList<EmbeddedTemplateExportItem> Discover(string projectRoot)
		{
			var assembly = Assembly.GetExecutingAssembly();
			var rootNamespace = assembly.GetName().Name!;
			var prefix = rootNamespace + "." + ResourceSegment;
			var templatesRoot = Path.Combine(projectRoot, ForgeFolder, TemplatesFolder);

			var items = new List<EmbeddedTemplateExportItem>();

			foreach (var resourceName in assembly.GetManifestResourceNames())
			{
				if (!resourceName.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
					continue;
				if (!resourceName.EndsWith(".cshtml", StringComparison.OrdinalIgnoreCase))
					continue;
				...
				items.Add(new EmbeddedTemplateExportItem
				{
		...
		public async Task<EmbeddedTemplateExportResult> ExportAsync(
			string projectRoot,
			bool force,
			bool whatIf,
			CancellationToken cancellationToken)
		{
			var result = new EmbeddedTemplateExportResult();
			var assembly = Assembly.GetExecutingAssembly();
