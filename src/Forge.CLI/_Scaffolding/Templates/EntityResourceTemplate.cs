...
{
    public class EntityResourceTemplate
    {
		public static ForgeFile Render(ForgeProject project, string contextName, ForgeContext context, string entityName, ForgeEntity entity, string culture = "")
		{
			...
			var filePath = $"src/{project.Name}.Core.Domain/{contextName}/Aggregates/{entityCollectionName}/Resources";
			var fileName = $"Entities{entityCollectionName}{(!string.IsNullOrWhiteSpace(culture) ? $".{culture}" : "")}.resx";
...
<root>
  ...
  {RenderRelations(entity)}
		...
		private static string RenderProperties(ForgeEntity entity)
		{
			...
			{
				result += @$"<data name=""{property.Key}"" xml:space=""preserve"">
    <value>{property.Key}</value>
		...
		private static string RenderRelations(ForgeEntity entity)
		{
			...
			{
				result += @$"<data name=""{relation.Key}"" xml:space=""preserve"">
